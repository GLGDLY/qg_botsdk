...
from ._utils import exception_processor, check_func
from .api import API
...
class BOT:
    ...
    def __init__(self, bot_id: str, bot_token: str, bot_secret: Optional[str] = None, is_private: bool = False,
        ...
        self._session = Session(self._loop, is_retry, is_log_error, self.logger, headers=self.bot_headers)
        self._commands = {}
        self._regex_commands = {}
        self.msg_treat = True
        self.dm_treat = False
        self.no_permission_warning = no_permission_warning
        self.max_workers = max_workers
        self.is_async = is_async
        self.api: Union[AsyncAPI, API] = AsyncAPI(self.bot_url, bot_id, bot_secret, self._session,
                                                  self.logger, self._check_warning)
        if not is_async:
            self.lock = TLock()
    ...
    def bind_msg(self, on_msg_function: Callable[[Model.MESSAGE], Any] = None, treated_data: bool = True,
                 all_msg: bool = None):
        """
    ...
    def bind_dm(self, on_dm_function: Callable[[Model.DIRECT_MESSAGE], Any] = None, treated_data: bool = True):
        """
        用作绑定接收私信消息的函数
    ...
    def bind_msg_delete(self, on_delete_function: Callable[[Model.MESSAGE_DELETE], Any] = None,
    ...
    def bind_guild_event(self, on_guild_event_function: Callable[[Model.GUILDS], Any] = None):
        """
        用作绑定接收频道信息的函数
    ...
    def bind_channel_event(self, on_channel_event_function: Callable[[Model.CHANNELS], Any] = None):
        """
        用作绑定接收子频道信息的函数
    ...
    def bind_guild_member(self, on_guild_member_function: Callable[[Model.GUILD_MEMBERS], Any] = None):
        """
        用作绑定接收频道成员信息的函数
    ...
    def bind_reaction(self, on_reaction_function: Callable[[Model.REACTION], Any] = None):
        """
        用作绑定接收表情表态信息的函数
    ...
    def bind_interaction(self, on_interaction_function: Callable[[Model.INTERACTION], Any] = None):
        """
        用作绑定接收互动事件的函数
    ...
    def bind_audit(self, on_audit_function: Callable[[Model.MESSAGE_AUDIT], Any] = None):
        """
        用作绑定接收审核事件的函数
    ...
    def register_repeat_event(self, time_function: Callable[[], Any] = None, check_interval: Union[float, int] = 10):
        """
    ...
    def start(self):
        ...
        try:
            if not self.__running and not self._bot_class:
                self.__running = True
                gateway = self._loop.run_until_complete(self._session.get(f'{self.bot_url}/gateway/bot'))
                ...
                self._bot_class = BotWs(self._session, self.logger, self._total_shard, self._shard_no, url,
                                        self.auth, self._on_msg_function, self._on_dm_function,
                                        self._on_delete_function, self.is_filter_self, self._on_guild_event_function,
                                        self._on_channel_event_function, self._on_guild_member_function,
                                        self._on_reaction_function, self._on_interaction_function,
                                        self._on_audit_function, self._on_forum_function, self._on_audio_function,
                                        self._intents, self.msg_treat, self.dm_treat, self._on_start_function,
                                        self.is_async, self.max_workers, self.api, self._commands, self._regex_commands)
