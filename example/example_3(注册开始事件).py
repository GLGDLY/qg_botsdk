    ...
    for items in all_guilds:
        ...
        if not gi.result and gi.data.code == 11292:
            ...
            if not all(gm.result):
                bot.logger.warning(items.name + '(' + items.id + ')[权限不足，无法查询此频道信息]')
                bot.api.create_permission_demand(items.id, 'xxx', 'get_guild_members', '查看成员权限')
            else:
            ...
            if not gr.result:
                bot.logger.warning(items.name + '(' + items.id + ')[权限不足，无法查询此频道信息]')
                bot.api.create_permission_demand(items.id, 'xxx', 'get_guild_roles', '查看成员权限')
            else:
            ...
            bot.logger.info(f'频道 {items.name}({items.id}) 子频道：' +
